...
const final_report_path = path.join(reports_directory, 'bad_soursing_trace.json');
const MAX_DEPTH = 2;
...
async function _processAddress(
    ...
    await _processOperations(prevLvlAddress, depth, position, timestamp, requestAddressSet);
}

async function _processOperations(address: string, depth: number, position: string, timestamp: number, requestAddressSet: Set<string>) {
    ...
    for (const tx of transactions) {
        ...
        await _prepareRecursion(position, address, depth, tx, requestAddressSet);
    }
}

async function _processTransactions(address: string, timestamp: number): Promise<ITransaction[]>{
...
async function _prepareRecursion(
    ...
    const addressInfo = await _getAddressInfo(tx.to);
    const countOfTransactions = Number(addressInfo.transactionCount);
    const isNotContract = addressInfo.contractAddress === '';
    console.log(`Address ${tx.from} has transactions: ${countOfTransactions}`);
    ...
    if (depth < MAX_DEPTH && attribution === null && countOfTransactions < 1000 && isNotContract) {
        await _processAddress(position, nextHopAddress, nextHopDepth, Number(tx.transactionTime), requestAddressSet);
    }
}
...
async function _saveStackToFile(stack: ITransaction[], attribution: Label, directory: string) {
    ...
    const csvWriter = createObjectCsvWriter({
        path: path.join(directory, `mid_report.csv`),
        header: [
    ...
    if (stack[2]) {
        record = {
            ...
            tx1_currency: stack[2]?.symbol || 'ETH',
            ...
            tx2_currency: stack[1]?.symbol || 'ETH',
            ...
            tx3_currency: stack[0]?.symbol || 'ETH',
...
async function _sendRequest(
    ...
    do {
        try{
            const response = await axios.get<ITxListResponse>('https://www.oklink.com/api/v5/explorer/address/normal-transaction-list', {
                params: {
                    chainShortName,
                    address,
                    page: page.toString(),
...
async function _sortnFilterTransactions(transactions: ITransaction[], timestamp: number): Promise<ITransaction[]> {
