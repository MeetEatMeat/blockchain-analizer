...
const inputDirectory = path.join(__dirname, '../inputs/transactions/eth');
...
const exchangeAddressesPath = path.join(__dirname, '../inputs/bitkub_addresses.csv');
...
function loadExchangeAddresses(): Set<string> {
  ...
  for (const record of records) {
    ...
    exchangeAddressSet.add(address.toLowerCase());
  }
...
function determineDirection(transactions: ITransaction[] | ITokenTransfer[]): 'forward' | 'backward' {
  let lastTransaction = transactions[transactions.length - 1];
  if (!lastTransaction) {
    lastTransaction = transactions[transactions.length - 2];
  }
  const fromAddress = lastTransaction?.from || '';
  const toAddress = lastTransaction?.to || '';

  if (exchangeAddresses.has(fromAddress)) {
    return 'backward';
  } else if (exchangeAddresses.has(toAddress)) {
    return 'forward';
  }
...
function isTokenTransfer(transaction: ITransaction | ITokenTransfer): transaction is ITokenTransfer {
  return (transaction as ITokenTransfer).tokenSymbol !== undefined;
}
...
function processJsonFilesToCsv(attributions: Map<string, string>) {
  ...
  for (const file of jsonFiles) {
    ...
    const transactions: ITokenTransfer[] | ITransaction[] = JSON.parse(rawData);
    ...
    if (transactions[2]) {
      if (direction === 'backward') {
        record = {
          ...
          tx1_currency: isTokenTransfer(transactions[2]) ? transactions[2].tokenSymbol || 'ETH' : 'ETH',
          ...
          tx2_currency: isTokenTransfer(transactions[1]) ? transactions[1].tokenSymbol || 'ETH' : 'ETH',
          ...
          tx3_currency: isTokenTransfer(transactions[0]) ? transactions[0].tokenSymbol || 'ETH' : 'ETH',
        ...
        record = {
          ...
          from_address_attribution: attributions.get(transactions[0]?.from.toLowerCase()) || '',
          tx1_hash: transactions[0]?.hash || '',
          tx1_timestamp: transactions[0]?.timeStamp || '',
          tx1_value: transactions[0]?.value || '',
          tx1_currency: isTokenTransfer(transactions[0]) ? transactions[0].tokenSymbol || 'ETH' : 'ETH',
          middle_address1: transactions[0]?.to || '',
          middle_address1_attribution: attributions.get(transactions[0]?.to.toLowerCase()) || '',
          tx2_hash: transactions[1]?.hash || '',
          tx2_timestamp: transactions[1]?.timeStamp || '',
          tx2_value: transactions[1]?.value || '',
          tx2_currency: isTokenTransfer(transactions[1]) ? transactions[1].tokenSymbol || 'ETH' : 'ETH',
          middle_address2: transactions[1]?.to || '',
          middle_address2_attribution: attributions.get(transactions[1]?.to.toLowerCase()) || '',
          tx3_hash: transactions[2]?.hash || '',
          tx3_timestamp: transactions[2]?.timeStamp || '',
          tx3_value: transactions[2]?.value || '',
          tx3_currency: isTokenTransfer(transactions[2]) ? transactions[2].tokenSymbol || 'ETH' : 'ETH',
          to_address: transactions[2]?.to || '',
          to_address_attribution: attributions.get(transactions[2]?.to.toLowerCase()) || ''
        };
      }
    } else {
      if (direction === 'backward') {
        record = {
          ...
          tx1_currency: isTokenTransfer(transactions[1]) ? transactions[1].tokenSymbol || 'ETH' : 'ETH',
          ...
          tx2_currency: isTokenTransfer(transactions[0]) ? transactions[0].tokenSymbol || 'ETH' : 'ETH',
          to_address: transactions[0]?.to || '',
          to_address_attribution: attributions.get(transactions[0]?.to.toLowerCase()) || '',
          middle_address2: '',
        ...
        record = {
          ...
          to_address_attribution: attributions.get(transactions[1]?.to.toLowerCase()) || '',
          middle_address2: '',
          middle_address2_attribution: '',
          tx3_hash: '',
          tx3_timestamp: '',
