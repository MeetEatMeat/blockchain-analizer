...
async function _processAddress(
    ...
    await _processTransactions(prevLvlAddress, depth, position, direction, timestamp, targetAddressSet, requestAddressSet);
}

async function _processTransactions(address: string, depth: number, position: string, direction: 'forward' | 'backward' | 'start', timestamp: number, targetAddressSet: Set<string>, requestAddressSet: Set<string>) {
    ...
    try{
        ...
        if(!Array.isArray(result)) {
            ...
            await _processAddress(position, address, depth, direction, timestamp, targetAddressSet, requestAddressSet);
            return;
        }
    } catch(e) {
    ...
    for (const tx of transactions) {
        ...
        await _prepareRecursion(position, address, direction, depth, tx, targetAddressSet, requestAddressSet);
    }
}
