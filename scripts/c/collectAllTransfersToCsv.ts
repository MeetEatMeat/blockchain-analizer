...
async function _sendRequest(
    ...
    do {
        const response = await axios.get<TxListResponse>('https://www.oklink.com/api/v5/explorer/address/token-transaction-list', {
            params: {
                chainShortName,
                address,
...
async function getTransactionList(address: string, chainShortName: string) {
    try {
        ...
        const csvWriter = createObjectCsvWriter({
            path: path.join(__dirname, 'transfers.csv'),
            header: [
                { id: 'from', title: 'from' },
