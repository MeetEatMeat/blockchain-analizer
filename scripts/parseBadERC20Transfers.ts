...
async function getTransactionList() {
    ...
    try {
        requestAddresses = await readAddressesFromCsv(path.join(__dirname, '../inputs/filtered_bad_eth.csv'));
    ...
    for (let i = 0; i < requestAddresses.length; i++) {
        ...
        await processAddress(position, requestAddresses[i].toLowerCase(), 0, 'start', null, 0);
    }
    ...
    async function processAddress(
        ...
        if (transactionCache.has(address)) {
            ...
            let response = await requestTTList_OkLink(address);
            ...
            if (response === null || response.status !== "1") {
                ...
                await processAddress(position, address, depth, direction, tokenSymbol, timestamp);
                return;
            }
        
            if (response.result.length > 99) {
                response = await requestTTList_Etherscan(address);
        ...
        for (const tx of transactions) {
            ...
            if (direction !== 'start' && tokenSymbol !== tx.tokenSymbol){
                console.log(`Wrong token symbol. Previously ${tokenSymbol} but now ${tx.tokenSymbol}`);
                continue;
            }
            ...
            console.log(`Processing tx for ${address} | Total: ${totalTransactions} | ${position} | Tuple length: ${txPath.length} | Depth: ${depth} | Bad txs: ${results.length} | ${direction} | ${tx.timeStamp} | ${tokenSymbol}`);
            ...
            if (tx.to.toLowerCase() === address && direction === 'backward'){
                ...
                if (depth < MAX_DEPTH) {
                    ...
                    await processAddress(position, tx.from.toLowerCase(), depth + 1, 'backward', tokenSymbol, Number(tx.timeStamp));
                }
                ...
                if (depth < MAX_DEPTH) {
                    ...
                    await processAddress(position, tx.to.toLowerCase(), depth + 1, 'forward', tokenSymbol, Number(tx.timeStamp));
                }
            } else if (tx.to.toLowerCase() === address && direction === 'start'){
                console.log(`3Recursion on ${depth + 1} | direction: ${direction}`);
                await processAddress(position, tx.from.toLowerCase(), depth + 1, 'backward', tx.tokenSymbol, Number(tx.timeStamp));
            } else if (tx.from.toLowerCase() === address && direction === 'start'){
                console.log(`4Recursion on ${depth + 1} | direction: ${direction}`);
                await processAddress(position, tx.to.toLowerCase(), depth + 1, 'forward', tx.tokenSymbol, Number(tx.timeStamp));
            }
        }
    }
...
async function requestTTList_OkLink(address: string) {
...
async function requestTTList_Etherscan(address: string) {
